...
class TurtleBot2Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self):
        ...
        * /kobuki/laser/scan: Laser Readings
        ...
        #self.gazebo.unpauseSim()  #### Gazebo 
        #self.controllers_object.reset_controllers()
        self._check_all_sensors_ready()

        # We Start all the ROS related Subscribers and publishers
        #rospy.Subscriber("/odom", Odometry, self._odom_callback)  #Gazebo
        #rospy.Subscriber("/odom_rf2o", Odometry, self._odom_callback)  #real_robot with rf2o
        rospy.Subscriber("/RosAria/pose", Odometry, self._odom_callback)  #real_robot
        ...
        #rospy.Subscriber("/camera/rgb/image_raw", Image, self._camera_rgb_image_raw_callback) Gazebo

        #rospy.Subscriber("/kobuki/laser/scan", LaserScan, self._laser_scan_callback)  #Gazebo
        rospy.Subscriber("/scan", LaserScan, self._laser_scan_callback)  #real_lidar
        #rospy.Subscriber("/scan_filtered", LaserScan, self._laser_scan_callback) #Bo loc Lidar

        #self._cmd_vel_pub = rospy.Publisher('/cmd_vel', Twist, queue_size=1) #Gazebo
        self._cmd_vel_pub = rospy.Publisher('/RosAria/cmd_vel', Twist, queue_size=1)  #Real_robot
    ...
    def _check_odom_ready(self):
        ...
        while self.odom is None and not rospy.is_shutdown():
            try:
                #self.odom = rospy.wait_for_message("/odom", Odometry, timeout=5.0) # Gazebo 
                #self.odom = rospy.wait_for_message("/odom_rf2o", Odometry, timeout=5.0) # real environment 
                self.odom = rospy.wait_for_message("/RosAria/pose", Odometry, timeout=5.0) # real environment 
    ...
    def _check_laser_scan_ready(self):
        ...
        while self.laser_scan is None and not rospy.is_shutdown():
            try:
                #self.laser_scan = rospy.wait_for_message("/kobuki/laser/scan", LaserScan, timeout=5.0) #Gazebo 
                self.laser_scan = rospy.wait_for_message("/scan", LaserScan, timeout=5.0)  # real environment 
